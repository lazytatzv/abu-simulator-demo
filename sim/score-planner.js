(function (root, factory) {
  ...
  function context(v, options = {}) {
    ...
    const home = F.points[v.team].home;
    ...
    return { motion, home, origin: options.origin || home, spots, target, travel, legal, options, remaining: Math.max(0, 180 - (v.time ?? v.observation.at) - (options.efficient ? 1 : 0)), candidates: 0 };
  }
    ...
    for (const option of pickups(state, ctx)) {
      ...
      function visit(current, position, tasks, taskTimes, seconds, budget) {
        for (const action of choices(current, ctx, budget)) {
          ...
          const candidate = { state: next, tasks: sequence, taskTimes: times, picked: option.picked, completeSeconds,
      ...
      visit(option.state, option.picked.length ? ctx.home : origin, [], [], option.prep, Math.max(1, option.state.cargo.length));
    }
    ...
    for (let depth = 0; depth < actions.length; depth++) {
      for (const old of [...paths.values()].filter(p => p.tasks.length === depth)) {
        ...
        for (let i = 0; i < actions.length; i++) {
          ...
          const candidate = { mask, last: i, state, tasks: sequence, taskTimes: [...old.taskTimes, seconds], picked: [], completeSeconds: seconds,
            cycleSeconds: seconds + ctx.travel(to, ctx.home) + ctx.motion.scanSeconds,
