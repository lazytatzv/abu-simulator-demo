...
test('finite study covers all forty-five profiles and all ordered pairs at five speed conditions', () => {
  assert.equal(Core.profiles().length, 45);
  const cases = Core.scenarios(); assert.equal(cases.length, 10125); assert.equal(new Set(cases.map(s => s.id)).size, 10125);
  for (const [r, b] of [[1, 1], [.75, 1], [1, .75], [.5, 1], [1, .5]]) assert.equal(cases.filter(s => s.config.redSpeed === r && s.config.blueSpeed === b).length, 2025);
});
