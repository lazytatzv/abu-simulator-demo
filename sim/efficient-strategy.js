(function (root, factory) {
  ...
  const homeScan = v => [go(F.points[v.team].home, '見渡し場所へ'), { type: 'scan' }];
  ...
  function stockDemand(v) {
    ...
    for (let i = v.cargo.length; i < 3; i++) {
      ...
      types.sort((a, b) => v.brPlan === 'earth-late' && v.time < 150 ? (a === 'earth' ? -1 : 1)
  ...
  function execute(v, selected, note = '') {
    ...
    if (selected.picked.length) return { brain: { stage: 'choose' }, decision,
      actions: [go(F.points[v.team].transferBR, 'まとめ受取へ'), ...selected.picked.map(objectId => ({ type: 'receive', objectId })), ...homeScan(v)] };
  ...
  function needsScan(v) { return v.failure || !v.observation || ['start', 'return'].includes(v.brain.stage) || !near(v, F.points[v.team].home, .12); }
  function returnCargo(v, note = '置けない荷物を種類別置場へ返却') {
    ...
    return { brain: { stage: 'choose' }, status: note, actions: [go(F.points[v.team].transferBR, note), ...cargo.map(o => ({ type: 'return', objectId: o.id })), ...homeScan(v)] };
  }
  function handoffNext(v) {
    if (needsScan(v)) return null;
    if (!v.cargo.length && v.observation.sanctuary && v.observation.handoff) return {
      brain: { stage: 'loaded' }, actions: [go(F.points[v.team].transferBR, 'MustikaをTRから直接受取'), { type: 'receive', objectId: 'M' }, ...homeScan(v)],
    };
    return null;
  }
  ...
  function builder(v, selectPlan = defaultPlan, options = {}) {
    ...
    if (obs.sanctuary && !v.cargo.length && (incoming || sourceMustika && !partner?.cargo.length)) {
      ...
      if (v.time - since < 18 && v.time < 163) return { brain: { ...v.brain, stage: 'return', mustikaWaitSince: since }, wait: 1, status: 'Mustika受取準備 · 空手で待機' };
    }
    ...
    if (!options.skipSupplyWait && (loading || canWaitForPair) && !incoming && v.time < 158) {
      ...
      if (v.time - since < 6) return { brain: { ...v.brain, stage: 'return', supplyWaitSince: since }, wait: 1, status: '補給待ち · まとめ受取の準備' };
    }
    ...
    return { brain: { stage: 'return' }, wait: 2, status: '2個の配置計画待ち · 補給・盤面を再確認' };
  }
